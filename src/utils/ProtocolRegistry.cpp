...
ProtocolRegistry::ProtocolRegistry() 
    : protocolName_("qclipboard")
    , protocolDescription_("QClipboard Protocol Handler") {
}
...
QString ProtocolRegistry::GetProtocolUrl(const QString& protocolName) {
  return QString("%1://").arg(protocolName);
}
...
void ProtocolRegistry::RegisterWinProtocol(const QString& protocolName) {
  ...
  settings.setValue(QString("%1/.").arg(commandKey),
                   value);
  
  qDebug() << "Windows protocol registered:" << protocolName;
}
...
void ProtocolRegistry::UnregisterWinProtocol(const QString& protocolName) {
  ...
  qDebug() << "Windows protocol unregistered:" << protocolName;
}
